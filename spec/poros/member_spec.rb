...
RSpec.describe Member do
  describe "attributes" do
    it "has an id, username, color, profile picture, and initials" do
      member = Member.new({
        :user=>{:id=>66176739, :username=>"Christopher Crane", :color=>"#d60800", :profilePicture=>"https://attachments.clickup.com/profilePictures/66176739_QbA.jpg", :initials=>"CC"}
      })

      expect(member).to be_a(Member)
      expect(member.id).to eq(66176739)
